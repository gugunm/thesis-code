...
leskFasttextOutput_3_cw_2 = './assets/leskFasttextOutput_1_cw_2/ayat'
...
def createWordEmbeddingModel(data):
  ...
  for idx, i in enumerate(listIndexData):
    ...
    dfWordEmbed = pd.DataFrame(columns=listTerms, index=list(range(301))) #.astype(object)
    ...
    for term in listTerms:
      ...
      for j in list(range(len(dfConcept))):
        ...
        accumulatorTerm = accumulatorTerm + dfWeightedAyat.loc[j][term]
      
      # jadikan accumulator tipenya array, karena mau di concate dengan lesk vector
      avgAccumulatorTerm = [accumulatorTerm / len(dfConcept)]
      
      # gabungkan lesk vektor dan fasttext vektor
      leskAvgConcateFasttext = avgAccumulatorTerm + wordVector
      
      # hitung vector rata-rata untuk suatu index/term nya
      dfWordEmbed[term] = leskAvgConcateFasttext
    ...
    save_zipped_pickle(ayat, idx, leskFasttextOutput_3_cw_2)
  return 0
