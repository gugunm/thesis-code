...
leskFasttextOutput_1_cw_2 = './assets/leskFasttextOutput_1_cw_2/ayat'
...
def createWordEmbeddingModel(data):
  ...
  for idx, i in enumerate(listIndexData):
    ...
    dfWordEmbed = pd.DataFrame(columns=listTerms, index=list(range(300))) #.astype(object)

    # looping tiap terms
    for term in listTerms:
      # ambil vector terms-nya dari model fasttext
      wordVector = ft.get_word_vector(term)
      # create vector akumulator buat cari rata-rata vector buat tiap indexnya (panjangnya 300)
      vectorAccumulatorIndex = np.zeros(300)
      # looping tiap column
      for j in list(range(len(dfConcept))):
        # kalikan tiap embedding index dengan nilai lesk index di tiap column-nya
        leskKaliEmbedding = dfWeightedAyat.loc[j][term] * wordVector
        # tambahkan dengan vector akumulator
        vectorAccumulatorIndex = np.add(vectorAccumulatorIndex, leskKaliEmbedding)

      # hitung vector rata-rata untuk suatu index/term nya
      dfWordEmbed[term] = np.divide(vectorAccumulatorIndex, len(dfConcept))

      # print("Vector rata2 - {}".format(index), dfWordEmbed.loc[index])
      # print("Vector Embedding - {}".format(index), wordVectorIndex)
      # display(dfWordEmbed)
      # break
    ...
    save_zipped_pickle(ayat, idx, leskFasttextOutput_1_cw_2)
  return 0
